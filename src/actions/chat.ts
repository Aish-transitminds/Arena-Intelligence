...
async function askGemini(question: string, staticContext: string, liveContext: any, personaContext: string, lang: string) {
  const systemPrompt = `You are Arena IQ, the intelligent operations copilot for MetLife Stadium during the FIFA World Cup 2026.
${personaContext}
...
export const askGeminiRAG = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const answer = await askGemini(message, staticContext, liveContext, personaContext, lang);
